...
또는 `멀티 태스킹` 측면에서 바라본다면 가상 메모리는  
OS가 메모리를 N개의 프로세스에게 각각 어떻게 분배할 지 고민하고, 각자에게 독립된 메모리 공간을 할당하는데  
메인 메모리(RAM)와 보조 메모리(HDD)를 혼합하여 `연속된 하나의 메모리처럼 추상화`된 공간이라 볼 수 있다.  

따라서 프로세스에 속한 1개 이상의 모든 스레드들 또한  
프로세스에게 할당 된 가상 메모리로 공간을 제약 받고 동일한 공간을 공유하게 된다.  
> 번외) 스레드는 다른 스레드의 스택에 직접적인 접근이 불가능한 각자 별도의 `스택 메모리`를 가지고 있다.  
> 또한 스레드는 멀티 스레드 환경에서 전역 변수를 공유할 때 생기는 `동시성 문제`를 해결하기 위한 용도로 `스레드 로컬 스토리지(TLS)`라는 것을 별도로 가지고 있다.

가상 주소와 실제 주소간의 변환 과정인 Address Mapping을 통해  
...
- https://youtu.be/x-Lp-h_pf9Q?si=h9GS9pvB-enhcJ3w
